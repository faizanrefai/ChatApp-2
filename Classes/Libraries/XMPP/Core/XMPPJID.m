...
+ (XMPPJID *)jidWithPrevalidatedUser:(NSString *)user prevalidatedDomain:(NSString *)domain prevalidatedResource:(NSString *)resource {
	XMPPJID *jid = [[XMPPJID alloc] init];
	jid->user = [user copy];
	jid->domain = [domain copy];
	jid->resource = [resource copy];
...
+ (XMPPJID *)jidWithPrevalidatedUser:(NSString *)user prevalidatedDomain:(NSString *)domain resource:(NSString *)resource {
	NSString *prepResource = [LibIDN prepResource:resource];
	if (![self validateResource:prepResource]) return nil;
