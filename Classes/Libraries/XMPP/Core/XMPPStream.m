...
- (void)dealloc {
	...
	[[self class] performSelector:@selector(xmppThreadStop) onThread:xmppUtilityThread withObject:nil
	                waitUntilDone:NO];
}
...
- (void)continueSendIQ:(XMPPIQ *)iq withTag:(long)tag {
	...
	[asyncSocket writeData:outgoingData withTimeout:TIMEOUT_XMPP_WRITE tag:tag];
...
- (void)sendOpeningNegotiation {
	...
	if (![self didStartNegotiation]) {
		...
		[asyncSocket writeData:outgoingData withTimeout:TIMEOUT_XMPP_WRITE tag:TAG_XMPP_WRITE_START];
				...
				dispatch_async(concurrentQueue, ^{ @autoreleasepool {
					...
					while ([delegateEnumerator getNextDelegate:&delegate delegateQueue:&dq forSelector:selector]) {
						dispatch_sync(dq, ^{ @autoreleasepool {
							...
							NSString *delegateAlternativeResource = [delegate xmppStream:self alternativeResourceForConflictingResource:currentResource];
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element {
	...
	if (state == STATE_XMPP_NEGOTIATING) {
		...
		if ([elementName isEqualToString:@"iq"])
		{
			...
			[self receivePresence:[XMPPPresence presenceFromElement:element]];
		} else if ([self isP2P] && ([elementName isEqualToString:@"stream:features"] || [elementName isEqualToString:@"features"])) {
...
+ (void)xmppThreadMain {
	...
	@autoreleasepool {
		...
		[NSTimer scheduledTimerWithTimeInterval:[[NSDate distantFuture] timeIntervalSinceNow] target:self selector:@selector(xmppThreadIgnore:) userInfo:nil repeats:YES];
		...
		while (!isCancelled && hasRunLoopSources) {
			@autoreleasepool {
				...
				hasRunLoopSources = [[NSRunLoop currentRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate distantFuture]];
