...
+ (XMPPvCardCoreDataStorageObject *)fetchvCardForJID:(XMPPJID *)jid inManagedObjectContext:(NSManagedObjectContext *)moc {
...
+ (XMPPvCardCoreDataStorageObject *)insertEmptyvCardForJID:(XMPPJID *)jid inManagedObjectContext:(NSManagedObjectContext *)moc {
	NSString *entityName = NSStringFromClass([XMPPvCardCoreDataStorageObject class]);
...
+ (XMPPvCardCoreDataStorageObject *)fetchOrInsertvCardForJID:(XMPPJID *)jid inManagedObjectContext:(NSManagedObjectContext *)moc {
...
- (void)setPhotoData:(NSData *)photoData {
	...
	if (self.vCardAvatarRel == nil) {
		...
		self.vCardAvatarRel = [NSEntityDescription insertNewObjectForEntityForName:entityName inManagedObjectContext:[self managedObjectContext]];
	}
...
- (void)setVCardTemp:(XMPPvCardTemp *)vCardTemp {
	...
	if (self.vCardTempRel == nil) {
		...
		self.vCardTempRel = [NSEntityDescription insertNewObjectForEntityForName:entityName inManagedObjectContext:[self managedObjectContext]];
	}
