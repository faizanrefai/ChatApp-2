...
- (void)timeoutRemoveRpcID:(NSString *)elementID {
	...
	if (rpcID) {
		...
		NSError *error = [NSError errorWithDomain:XMPPJabberRPCErrorDomain code:1400 userInfo:[NSDictionary dictionaryWithObjectsAndKeys: @"Request timed out", @"error",nil]];
	...
	if ([iq isJabberRPC]) {
		if ([iq isResultIQ] || [iq isErrorIQ]) {
			...
			if ([iq isResultIQ]) {
				...
				NSError *error = [NSError errorWithDomain:XMPPJabberRPCErrorDomain code:[errorElement attributeIntValueForName:@"code"] userInfo:[NSDictionary dictionaryWithObjectsAndKeys: [errorElement attributesAsDictionary],@"error", [[errorElement childAtIndex:0] name], @"condition", iq,@"iq", nil]];
