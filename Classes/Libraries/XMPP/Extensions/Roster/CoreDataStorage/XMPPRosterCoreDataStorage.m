...
- (void)_clearAllResourcesForXMPPStream:(XMPPStream *)stream {
	...
	if (stream) {
		...
		predicate = [NSPredicate predicateWithFormat:@"streamBareJidStr == %@", [[self myJIDForXMPPStream:stream] bare]];
...
- (XMPPResourceCoreDataStorageObject *)resourceForJID:(XMPPJID *)jid xmppStream:(XMPPStream *)stream managedObjectContext:(NSManagedObjectContext *)moc {
	...
	NSEntityDescription *entity = [NSEntityDescription entityForName:@"XMPPResourceCoreDataStorageObject" inManagedObjectContext:moc];
...
- (void)handleRosterItem:(NSXMLElement *)itemSubElement xmppStream:(XMPPStream *)stream {
	...
	[self scheduleBlock:^{
		...
		if ([rosterPopulationSet containsObject:[NSNumber numberWithPtr:(__bridge void *)stream]]) {
			...
			[XMPPUserCoreDataStorageObject insertInManagedObjectContext:moc withItem:item streamBareJidStr:streamBareJidStr];
		} else {
...
- (void)handlePresence:(XMPPPresence *)presence xmppStream:(XMPPStream *)stream {
	...
	[self scheduleBlock:^{
		...
		if (user == nil) {
			...
			user = [XMPPUserCoreDataStorageObject insertInManagedObjectContext:moc withJID:[presence from] streamBareJidStr:streamBareJidStr];
		}
