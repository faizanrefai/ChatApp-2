...
+ (id)insertInManagedObjectContext:(NSManagedObjectContext *)moc withJID:(XMPPJID *)jid streamBareJidStr:(NSString *)streamBareJidStr {
	if (jid == nil) {
	...
	newUser = [NSEntityDescription insertNewObjectForEntityForName:@"XMPPUserCoreDataStorageObject" inManagedObjectContext:moc];
...
+ (id)insertInManagedObjectContext:(NSManagedObjectContext *)moc withItem:(NSXMLElement *)item streamBareJidStr:(NSString *)streamBareJidStr {
...
- (void)updateGroupsWithItem:(NSXMLElement *)item {
	...
	for (NSXMLElement *groupElement in groupItems) {
		...
		group = [XMPPGroupCoreDataStorageObject fetchOrInsertGroupName:groupName inManagedObjectContext:[self managedObjectContext]];
...
- (void)updateWithPresence:(XMPPPresence *)presence streamBareJidStr:(NSString *)streamBareJidStr {
	...
	if ([[presence type] isEqualToString:@"unavailable"] || [presence isErrorPresence]) {
		...
		if (resource) {
			...
			newResource = [XMPPResourceCoreDataStorageObject insertInManagedObjectContext:[self managedObjectContext] withPresence:presence streamBareJidStr:streamBareJidStr];
...
**/
- (NSComparisonResult)compareByAvailabilityName:(XMPPUserCoreDataStorageObject *)another options:(NSStringCompareOptions)mask {
	if ([self isOnline]) {
		if ([another isOnline])
