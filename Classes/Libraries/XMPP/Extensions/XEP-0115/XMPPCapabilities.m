...
- (void)fetchCapabilitiesForJID:(XMPPJID *)jid {
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		[xmppCapabilitiesStorage getCapabilitiesKnown:&areCapabilitiesKnown failed:&haveFailedFetchingBefore node:&node ver:&ver ext:nil hash:&hash algorithm:&hashAlg forJID:jid xmppStream:xmppStream];
...
- (void)handlePresenceCapabilities:(NSXMLElement *)c fromJID:(XMPPJID *)jid {
	...
	BOOL areCapabilitiesKnown = [xmppCapabilitiesStorage setCapabilitiesNode:node ver:ver ext:nil hash:ver algorithm:hash forJID:jid xmppStream:xmppStream andGetNewCapabilities:&newCapabilities];
	if (areCapabilitiesKnown) {
...
- (void)handleLegacyPresenceCapabilities:(NSXMLElement *)c fromJID:(XMPPJID *)jid {
	...
	BOOL areCapabilitiesKnown = [xmppCapabilitiesStorage setCapabilitiesNode:node ver:ver ext:ext hash:nil algorithm:nil forJID:jid xmppStream:xmppStream andGetNewCapabilities:nil];
	if (areCapabilitiesKnown) {
...
- (void)handleDiscoRequest:(XMPPIQ *)iqRequest {
	...
	if (myCapabilitiesQuery == nil) {
		...
		XMPPIQ *iqResponse = [XMPPIQ iqWithType:@"result" to:[iqRequest from] elementID:[iqRequest elementID] child:query];
...
- (void)handleDiscoResponse:(NSXMLElement *)querySubElement fromJID:(XMPPJID *)jid {
	...
	BOOL hashResponse = [xmppCapabilitiesStorage getCapabilitiesHash:&hash algorithm:&hashAlg forJID:jid xmppStream:xmppStream];
	if (hashResponse) {
...
- (void)maybeQueryNextJidWithHashKey:(NSString *)key dueToHashMismatch:(BOOL)hashMismatch {
	...
	if (requestIndex < [jids count]) {
		...
		[xmppCapabilitiesStorage getCapabilitiesKnown:nil failed:nil node:&node ver:&ver ext:nil hash:nil algorithm:nil forJID:jid xmppStream:xmppStream];
