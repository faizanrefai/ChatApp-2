...
- (void)addMessage:(XMPPRoomMessageMemoryStorageObject *)roomMsg {
	...
	[[self multicastDelegate] xmppRoomMemoryStorage:self didReceiveMessage:roomMsgCopy fromOccupant:occupantCopy atIndex:index inArray:messagesCopy];
}
...
- (void)handlePresence:(XMPPPresence *)presence room:(XMPPRoom *)room {
	...
	if ([[presence type] isEqualToString:@"unavailable"]) {
		...
		if (occupant) {
			...
			[[self multicastDelegate] xmppRoomMemoryStorage:self occupantDidLeave:occupantCopy atIndex:index fromArray:occupantsCopy];
		}
	} else {
		XMPPRoomOccupantMemoryStorageObject *occupant = [occupantsDict objectForKey:from];
		if (occupant == nil) {
			...
			[[self multicastDelegate] xmppRoomMemoryStorage:self occupantDidJoin:occupantCopy atIndex:index inArray:occupantsCopy];
		} else {
			...
			[[self multicastDelegate] xmppRoomMemoryStorage:self occupantDidUpdate:occupantCopy fromIndex:oldIndex toIndex:newIndex inArray:occupantsCopy];
		}
	}
}
