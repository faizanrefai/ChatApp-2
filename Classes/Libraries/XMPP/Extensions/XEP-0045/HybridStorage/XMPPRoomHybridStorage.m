...
- (void)performDelete {
	...
	if ([pausedMessageDeletion count] > 0) {
		predicate = [NSPredicate predicateWithFormat:@"localTimestamp <= %@ AND roomJIDStr NOT IN %@", minLocalTimestamp, pausedMessageDeletion];
	} else {
...
-(BOOL)existsMessage:(XMPPMessage *)message forRoom:(XMPPRoom *)room stream:(XMPPStream *)xmppStream {
	...
	NSPredicate *predicate = [NSPredicate predicateWithFormat:predicateFormat, messageBody, messageJID, streamBareJidStr, remoteTimestamp, minLocalTimestamp, maxLocalTimestamp];
...
- (void)insertMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing forRoom:(XMPPRoom *)room stream:(XMPPStream *)xmppStream {
	...
	XMPPRoomMessageHybridCoreDataStorageObject *roomMessage = (XMPPRoomMessageHybridCoreDataStorageObject *) [[NSManagedObject alloc] initWithEntity:messageEntity insertIntoManagedObjectContext:nil];
...
**/ 
- (XMPPRoomOccupantHybridMemoryStorageObject *)insertOccupantWithPresence:(XMPPPresence *)presence                                                                      room:(XMPPRoom *)room stream:(XMPPStream *)xmppStream {
...
**/
- (void)updateOccupant:(XMPPRoomOccupantHybridMemoryStorageObject *)occupant withPresence:(XMPPPresence *)presence room:(XMPPRoom *)room stream:(XMPPStream *)stream {
...
**/
- (void)removeOccupant:(XMPPRoomOccupantHybridMemoryStorageObject *)occupant withPresence:(XMPPPresence *)presence room:(XMPPRoom *)room stream:(XMPPStream *)stream {
...
- (NSDate *)mostRecentMessageTimestampForRoom:(XMPPJID *)roomJID stream:(XMPPStream *)xmppStream                                     inContext:(NSManagedObjectContext *)inMoc {
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		NSError *error = nil;
		XMPPRoomMessageHybridCoreDataStorageObject *message = (XMPPRoomMessageHybridCoreDataStorageObject *) [[moc executeFetchRequest:fetchRequest error:&error] lastObject];
		...
		if ([[presence type] isEqualToString:@"unavailable"]) {
			...
			if (occupant) {
				...
				dispatch_async(roomQueue, ^{ @autoreleasepool {
					...
					[roomMulticastDelegate xmppRoomHybridStorage:self occupantDidLeave:occupantCopy];
				}});
			}
		} else {
			XMPPRoomOccupantHybridMemoryStorageObject *occupant = [self occupantForJID:from stream:xmppStream];
			if (occupant == nil) {
				...
				dispatch_async(roomQueue, ^{ @autoreleasepool {
					...
					[roomMulticastDelegate xmppRoomHybridStorage:self occupantDidJoin:occupantCopy];
				}});
			} else {
				...
				dispatch_async(roomQueue, ^{ @autoreleasepool {
					...
					[roomMulticastDelegate xmppRoomHybridStorage:self occupantDidUpdate:occupantCopy];
				}});
			}
		}
	}];
